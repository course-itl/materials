...
## Что мы вообще собираем и почему

Три вещи, чтобы дальнейшие шаги не были магией:

**Почему Unix/Linux.** Сервер курса — Linux, и почти всё серверное в мире — Linux. Программы, которые вы будете писать, будут жить на Linux; значит, и разрабатывать удобнее в среде, которая ведёт себя так же. Windows сама по себе устроена иначе — поэтому на Windows мы ставим WSL2, а macOS подходит почти как есть (это тоже Unix, кузен Linux).

**Что такое пакетный менеджер.** Программы в Unix не скачивают с сайтов по одной — их ставит специальная программа-завхоз: `apt` в Ubuntu, `brew` на macOS. Одна команда — и инструмент установлен, зависимости подтянуты, обновления централизованы. Все команды вида `sudo apt install ...` ниже — это именно оно. (`sudo` впереди — «выполни от имени администратора»: установка программ меняет систему, на это нужны права.)

**Что мы ставим.** Набор инструментов на весь курс, чтобы больше к этому не возвращаться:

| Инструмент | Зачем (и когда выстрелит) |
|---|---|
| `build-essential` / компиляторы | собирать программы из исходников (C — с недели 5) |
| `gdb` | отладчик (неделя 5+) |
| `git` | контроль версий (урок 5, всерьёз — недели 3–4) |
| `python3` | скрипты и прототипы (неделя 2+) |
| `tmux` | терминальные сессии, переживающие обрыв связи (урок 4) |
| `curl`, `wget` | скачивание из терминала |
| `tree` | красивая печать дерева каталогов |
| `man-db` | справочник `man` — главный источник ответов на курсе |
...
WSL2 — это полноценный Linux внутри Windows: не «эмуляция команд», а настоящее Linux-ядро в лёгкой виртуальной машине, которую Windows запускает и обслуживает сама. Именно так на Windows работает большинство разработчиков. Почему не VirtualBox и не второй системой на диске: WSL2 легче, быстрее стартует, официально поддерживается Microsoft и не требует перекраивать диск — а результат для нас тот же, честный Linux.
...
Финальный шаг. `check-env.sh` — скрипт, который проверяет ваше окружение по списку: нужные программы установлены и находятся, версии не древние, базовые вещи работают. Зелёный `check-env.sh` означает «моя машина совместима с курсом» — дальше все материалы недели будут работать у вас так же, как у всех.
